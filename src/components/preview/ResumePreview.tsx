...
import { normalizeResumeData } from '@/lib/normalization/resume-normalizer';
...
export function ResumePreview() {
  ...
  // Normalize data and calculate intelligent pagination
  const sectionOrder = useMemo(() => getEffectiveSectionOrder(resumeData), [resumeData]);
  const normalizedResume = useMemo(() => resumeData ? normalizeResumeData(resumeData) : null, [resumeData]);
  const pagination = useMemo(() => {
    if (!normalizedResume) return null;
    return calculateResumePagination(normalizedResume, templateSettings, sectionOrder);
  }, [normalizedResume, templateSettings, sectionOrder]);
  ...
  if (!resumeData || !normalizedResume || !pagination) return null;
  ...
  const lineSpacingClass = {
    tight: 'leading-[1.26]',
    normal: 'leading-[1.32]',
    relaxed: 'leading-[1.38]',
  }[templateSettings?.lineSpacing || 'normal'];

  const accentColor = templateSettings?.accentColor || '#141413';
  ...
  const renderSectionContent = (secItem: PageSectionItem) => {
    const sId = secItem.sectionId;

    if (sId === 'personal_info') {
      return (
        <div className="relative group/header-wrapper">
          ...
          <PersonalInfoSection data={normalizedResume.personal_info} />
        </div>
      );
    }

    if (sId === 'work_experience' && normalizedResume.work_experience && normalizedResume.work_experience.length > 0) {
      const itemsToRender = secItem.items || normalizedResume.work_experience;
      return (
        <div className="relative group/section-wrapper">
          <WorkExperienceSection data={itemsToRender} />
        </div>
      );
    }

    if (sId === 'education' && normalizedResume.education && normalizedResume.education.length > 0) {
      const itemsToRender = secItem.items || normalizedResume.education;
      return (
        <div className="relative group/section-wrapper">
          <EducationSection data={itemsToRender} />
        </div>
      );
    }

    if (sId === 'skills_and_interests' && normalizedResume.skills_and_interests) {
      return (
        <div className="relative group/section-wrapper">
          <SkillsSection data={normalizedResume.skills_and_interests} />
        </div>
      );
    }

    // Custom Section
    const customSec = normalizedResume.custom_sections?.find(s => s.id === sId);
    if (customSec) {
      const idx = normalizedResume.custom_sections?.findIndex(s => s.id === sId) ?? 0;
      const sectionWithItems = secItem.items ? { ...customSec, items: secItem.items } : customSec;
      return (
        <div className="relative group/section-wrapper">
          <CustomSectionView section={sectionWithItems} sectionIndex={idx} />
        </div>
      );
    }
  ...
  return (
    <div ref={containerRef} className="flex flex-col items-center w-full max-w-full">
      ...
      <div className="w-full max-w-[8.5in] mb-4 flex items-center justify-between px-3.5 py-2 neo-box no-print text-xs font-mono gap-2">
        ...
        <div className="flex items-center gap-2">
          {pagination.totalPages > 1 ? (
            <div className="flex items-center gap-1.5 text-[#141413] bg-[#E2E8F0] px-2 py-0.5 border border-[#141413] font-black">
              <Layers className="w-3.5 h-3.5 text-[#141413]" />
              <span>BALANCED {pagination.totalPages}-PAGE DOCUMENT</span>
            </div>
          ) : isOverflowing ? (
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center border-2 border-[#141413] bg-white shadow-[1.5px_1.5px_0px_#141413] p-0.5">
            ...
            <button
              ...
              className="px-1.5 py-0.5 text-[11px] font-mono font-bold text-[#141413] hover:bg-[#F2EFE9] cursor-pointer"
              title="Reset Zoom"
            >
      ...
      <div 
        ...
        style={{
          height: paperHeight ? `${Math.ceil(paperHeight * effectiveZoom) + (pagination.totalPages > 1 ? 64 : 32)}px` : undefined
        }}
      >
        <div
          className="origin-top shrink-0 transition-transform duration-150 flex flex-col items-center"
          style={{
            width: '816px',
            transform: `scale(${effectiveZoom})`,
          }}
        >
          {pagination.pages.map((pg, pIdx) => {
            const isSingle = pagination.totalPages === 1;

            return (
              <div key={pg.pageNumber} className="relative flex flex-col items-center w-full mb-8 last:mb-0">
                {/* Physical Sheet Header Badge on Multi-Page Documents */}
                {!isSingle && (
                  <div className="w-full flex justify-between items-center text-[11px] font-mono text-[#76736C] mb-1.5 px-2 select-none no-print uppercase font-semibold">
                    <span>Sheet {pg.pageNumber} of {pg.totalNumber}</span>
                    <span>Standard Letter · 8.5" × 11"</span>
                  </div>
                )}

                <div 
                  id={pIdx === 0 ? 'resume-paper-element' : `resume-paper-page-${pg.pageNumber}`}
                  ref={pIdx === 0 ? paperRef : undefined}
                  data-page-number={pg.pageNumber}
                  className={`bg-white resume-paper resume-page-sheet editorial-document-shadow border-2 border-[#141413] rounded-none font-serif text-black ${fontSizeClass} ${lineSpacingClass} relative transition-all`}
                  style={{
                    width: '8.5in', // 816px
                    ...
                    paddingLeft: spacing.pagePaddingLeft,
                    paddingRight: spacing.pagePaddingRight,
                    fontFamily: currentFontFamily,
                  }}
                >
                  {/* Visual 1-Page Cutoff Guide Line for Single Page Mode */}
                  {isSingle && showPageBoundary && isOverflowing && (
                    <div 
                    ...
                    >
                      <div className="w-full border-t-2 border-dashed border-[#141413] relative flex items-center justify-center">
                        <div className="bg-[#FF6B4A] text-[#141413] border-2 border-[#141413] shadow-[2px_2px_0px_#141413] px-3 py-1 font-mono text-[10px] tracking-wider uppercase font-black flex items-center gap-1.5 transform -translate-y-1/2">
                          ...
                          <span>🚨 1-PAGE CUTOFF (SPILL: +{overflowPx}PX)</span>
                        </div>
                      </div>
                    </div>
                  )}

                  {/* Render sections allocated to this physical page */}
                  {pg.sections.map((secItem) => (
                    <React.Fragment key={`${secItem.sectionId}-${secItem.isContinuation ? 'cont' : 'main'}`}>
                      {renderSectionContent(secItem)}
                    </React.Fragment>
                  ))}

                  {/* Add Section Button on Last Page */}
                  {pIdx === pagination.pages.length - 1 && (
                    <div className="pt-3 pb-1 border-t border-dashed border-[#E7E4DC] mt-4 text-center no-print">
                      <button
                        onClick={() => setIsAddSectionOpen(true)}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 font-mono text-[10.5pt] tracking-wider uppercase font-semibold text-[#141413] hover:text-[#993322] bg-[#F8F7F4] hover:bg-white border border-[#E7E4DC] hover:border-[#141413] transition-colors cursor-pointer"
                      >
