...
export async function renderCoverLetterDocx(
  ...
  const makeRun = (text: string, opts: { bold?: boolean; italic?: boolean; size?: number; color?: string } = {}) => {
    const isB = !!opts.bold;
    const isI = !!opts.italic;
    const sz = opts.size || 22; // 11pt default
    const col = opts.color ? `<w:color w:val="${opts.color}"/>` : '';
    return `<w:r><w:rPr><w:rFonts w:ascii="${fontFamily}" w:cs="${fontFamily}" w:eastAsia="${fontFamily}" w:hAnsi="${fontFamily}"/>${isB ? '<w:b w:val="1"/><w:bCs w:val="1"/>' : ''}${isI ? '<w:i w:val="1"/><w:iCs w:val="1"/>' : ''}${col}<w:sz w:val="${sz}"/><w:szCs w:val="${sz}"/></w:rPr><w:t xml:space="preserve">${xmlEscape(text)}</w:t></w:r>`;
  };

  const makePara = (runsXml: string, opts: { align?: string; spaceBefore?: number; spaceAfter?: number; borderBottom?: boolean; lineSpacing?: number } = {}) => {
  ...
  paragraphs.push(
    makePara(makeRun(candidateName, { bold: true, size: 36 }), { align: 'center', spaceBefore: 0, spaceAfter: 40 })
  );
  ...
  if (contactParts.length > 0) {
    paragraphs.push(
      makePara(makeRun(contactParts.join('  |  '), { size: 19, color: '555555' }), {
        align: 'center',
        spaceBefore: 0,
        spaceAfter: 280,
        borderBottom: true
  ...
  // 3. Date
  const dateStr = letter.date || new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
  paragraphs.push(makePara(makeRun(dateStr, { size: 21 }), { spaceBefore: 120, spaceAfter: 200 }));
  ...
  recipientRuns.forEach((line, idx) => {
    paragraphs.push(
      makePara(makeRun(line, { bold: idx === 0, size: 21 }), {
        spaceBefore: 0,
  ...
  paragraphs.push(makePara(makeRun(salutation, { bold: true, size: 21 }), { spaceBefore: 0, spaceAfter: 180 }));

  // 6. Opening Paragraph
  if (letter.opening_paragraph) {
    paragraphs.push(makePara(makeRun(letter.opening_paragraph, { size: 21 }), { spaceBefore: 0, spaceAfter: 180, lineSpacing: 280 }));
  }

  // 7. Body Paragraphs
  for (const bodyPara of letter.body_paragraphs || []) {
    if (bodyPara) {
      paragraphs.push(makePara(makeRun(bodyPara, { size: 21 }), { spaceBefore: 0, spaceAfter: 180, lineSpacing: 280 }));
    }
  }

  // 8. Closing Paragraph
  if (letter.closing_paragraph) {
    paragraphs.push(makePara(makeRun(letter.closing_paragraph, { size: 21 }), { spaceBefore: 0, spaceAfter: 260, lineSpacing: 280 }));
  }

  // 9. Sign-off
  paragraphs.push(makePara(makeRun(letter.sign_off || 'Sincerely,', { size: 21 }), { spaceBefore: 0, spaceAfter: 400 }));

  // 10. Candidate Signature Name
  paragraphs.push(makePara(makeRun(candidateName, { bold: true, size: 21 }), { spaceBefore: 0, spaceAfter: 0 }));
